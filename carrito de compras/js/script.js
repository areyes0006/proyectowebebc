...
function agregarProducto(nombreProducto) {
  // Lista principal
  if (listaCarrito.children.length === 1 && listaCarrito.children[0].textContent.includes('No hay atracciones')) {
  ...
  // Lista offcanvas
  if (listaCarritoOffcanvas.children.length === 1 && listaCarritoOffcanvas.children[0].textContent.includes('No hay atracciones')) {
  ...
  if (items.length > 0) {
    ...
    setTimeout(() => {
      ...
      if (listaCarrito.children.length === 0) {
        ...
        liVacio.textContent = 'No hay atracciones';
